...
export default function Home() {
  ...
  if (!auth) return (
    <div style={{minHeight:'100vh',background:'#07090f',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Noto Sans KR',sans-serif"}}>
      <div style={{background:'#0d1117',border:'1px solid #1f2d42',borderRadius:'8px',padding:'40px 48px',textAlign:'center',minWidth:'320px'}}>
        <div style={{fontFamily:"'Rajdhani',sans-serif",fontSize:'24px',fontWeight:700,color:'#e8b84b',marginBottom:'8px'}}>⚔ 다비도의 내전</div>
        ...
        <input
          ...
          placeholder="비밀번호"
          autoFocus
          style={{width:'100%',boxSizing:'border-box',background:'#131a24',border:`1px solid ${pwError?'#ea4335':'#1f2d42'}`,color:'#c8d8ec',borderRadius:'4px',padding:'10px 14px',fontSize:'14px',fontFamily:'inherit',outline:'none',marginBottom:'8px'}}
        />
        {pwError && <div style={{fontSize:'12px',color:'#ea4335',marginBottom:'8px'}}>비밀번호가 틀렸습니다</div>}
        <button
          onClick={()=>{if(pwInput===PASSWORD){setAuth(true)}else{setPwError(true)}}}
          style={{width:'100%',padding:'10px',borderRadius:'4px',border:'none',background:'#e8b84b',color:'#0d1117',fontWeight:700,fontSize:'14px',fontFamily:'inherit',cursor:'pointer',marginTop:'4px'}}
        >입장</button>
      </div>
    </div>
  )
